import { Event } from '../../types';
import {
  convertEventToDateRange,
  findOverlappingEvents,
  isOverlapping,
  parseDateTime,
} from '../../utils/eventOverlap';

describe('parseDateTime', () => {
  it('2024-07-01 14:30을 정확한 Date 객체로 변환한다', () => {
    const result = parseDateTime('2024-07-01', '14:30');
    expect(result).toEqual(new Date('2024-07-01T14:30'));
  });

  it('잘못된 날짜 형식에 대해 Invalid Date를 반환한다', () => {
    const result = parseDateTime('test', '14:30');
    expect(result).toEqual(new Date('testT14:30'));
    expect(result.toString()).toBe('Invalid Date');
  });

  it('잘못된 시간 형식에 대해 Invalid Date를 반환한다', () => {
    const result = parseDateTime('2024-07-01', 'test');
    expect(result).toEqual(new Date('2024-07-01Ttest'));
    expect(result.toString()).toBe('Invalid Date');
  });

  it('날짜 문자열이 비어있을 때 Invalid Date를 반환한다', () => {
    const result = parseDateTime('', '14:30');
    expect(result).toEqual(new Date('T14:30'));
    expect(result.toString()).toBe('Invalid Date');
  });
});

describe('convertEventToDateRange', () => {
  it('일반적인 이벤트를 올바른 시작 및 종료 시간을 가진 객체로 변환한다', () => {
    const event: Event = {
      id: '1',
      title: 'New Year Celebration',
      date: '2024-01-01',
      startTime: '00:00',
      endTime: '01:00',
      description: 'Celebrate the New Year',
      location: 'Home',
      category: 'Holiday',
      repeat: { type: 'yearly', interval: 1 },
      notificationTime: 60,
    }

    const result = convertEventToDateRange(event);

    expect(result.start).toEqual(new Date('2024-01-01T00:00'));
    
    expect(result.end).toEqual(new Date('2024-01-01T01:00'));
  });

  it('잘못된 날짜 형식의 이벤트에 대해 Invalid Date를 반환한다', () => {
    const event: Event = {
      id: '1',
      title: 'New Year Celebration',
      date: 'test',
      startTime: '00:00',
      endTime: '01:00',
      description: 'Celebrate the New Year',
      location: 'Home',
      category: 'Holiday',
      repeat: { type: 'yearly', interval: 1 },
      notificationTime: 60,
    };

    const result = convertEventToDateRange(event);

    expect(result.start.toString()).toBe('Invalid Date');

    expect(result.end.toString()).toBe('Invalid Date');
  });

  it('잘못된 시간 형식의 이벤트에 대해 Invalid Date를 반환한다', () => {
    const event: Event = {
      id: '1',
      title: 'New Year Celebration',
      date: '2024-01-01',
      startTime: 'test',
      endTime: '01:00',
      description: 'Celebrate the New Year',
      location: 'Home',
      category: 'Holiday',
      repeat: { type: 'yearly', interval: 1 },
      notificationTime: 60,
    };

    const result = convertEventToDateRange(event);
    expect(result.start.toString()).toBe('Invalid Date');
    expect(result.end).toEqual(new Date('2024-01-01T01:00'));
  });
});

describe('isOverlapping', () => {
  it('두 이벤트가 겹치는 경우 true를 반환한다', () => {
    const events: Event[] = [
      {
        id: '1',
        title: 'New Year Celebration',
        date: '2024-01-01',
        startTime: '00:00',
        endTime: '01:00',
        description: 'Celebrate the New Year',
        location: 'Home',
        category: 'Holiday',
        repeat: { type: 'yearly', interval: 1 },
        notificationTime: 60,
      },
      {
        id: '2',
        title: 'BirthDay',
        date: '2024-01-01',
        startTime: '00:00',
        endTime: '01:00',
        description: 'birthday',
        location: 'Home',
        category: 'birthday',
        repeat: { type: 'yearly', interval: 1 },
        notificationTime: 60,
      },
    ];

    expect(isOverlapping(events[0], events[1])).toBe(true);
  });

  it('두 이벤트가 겹치지 않는 경우 false를 반환한다', () => {
    const events: Event[] = [
      {
        id: '1',
        title: 'New Year Celebration',
        date: '2024-01-01',
        startTime: '00:00',
        endTime: '01:00',
        description: 'Celebrate the New Year',
        location: 'Home',
        category: 'Holiday',
        repeat: { type: 'yearly', interval: 1 },
        notificationTime: 60,
      },
      {
        id: '2',
        title: 'BirthDay',
        date: '2024-01-03',
        startTime: '00:00',
        endTime: '01:00',
        description: 'birthday',
        location: 'Home',
        category: 'birthday',
        repeat: { type: 'yearly', interval: 1 },
        notificationTime: 60,
      },
    ];

    expect(isOverlapping(events[0], events[1])).toBe(false);
  });
});

describe('findOverlappingEvents', () => {
  const events: Event[] = [
    {
      id: '1',
      title: 'New Year Celebration',
      date: '2024-01-01',
      startTime: '00:00',
      endTime: '01:00',
      description: 'Celebrate the New Year',
      location: 'Home',
      category: 'Holiday',
      repeat: { type: 'yearly', interval: 1 },
      notificationTime: 60,
    },
    {
      id: '2',
      title: 'BirthDay',
      date: '2024-01-03',
      startTime: '00:00',
      endTime: '01:00',
      description: 'birthday',
      location: 'Home',
      category: 'birthday',
      repeat: { type: 'yearly', interval: 1 },
      notificationTime: 60,
    },
  ];

  it('새 이벤트와 겹치는 모든 이벤트를 반환한다', () => {
    const newEvent: Event = {
      id: '99',
      title: 'New Year Celebration',
      date: '2024-01-01',
      startTime: '00:00',
      endTime: '01:00',
      description: 'Celebrate the New Year',
      location: 'Home',
      category: 'Holiday',
      repeat: { type: 'yearly', interval: 1 },
      notificationTime: 60,
    };

    const overlappingEvents = findOverlappingEvents(newEvent, events);
    expect(overlappingEvents).toHaveLength(1);
    expect(overlappingEvents[0].id).toBe('1');
  });

  it('겹치는 이벤트가 없으면 빈 배열을 반환한다', () => {
    const newEvent: Event = {
      id: '99',
      title: 'Christmas',
      date: '2024-12-25',
      startTime: '00:00',
      endTime: '01:00',
      description: 'Christmas',
      location: 'Home',
      category: 'Holiday',
      repeat: { type: 'yearly', interval: 1 },
      notificationTime: 60,
    };

    const overlappingEvents = findOverlappingEvents(newEvent, events);
    expect(overlappingEvents).toHaveLength(0);
  });
});
