...
describe('getDaysInMonth', () => {
  it('1월은 31일 수를 반환한다', () => {
    const daysInJanuary = getDaysInMonth(2024, 1);
    expect(daysInJanuary).toBe(31);
  });

  it('4월은 30일 일수를 반환한다', () => {
    const daysInApril = getDaysInMonth(2024, 4);
    expect(daysInApril).toBe(30);
  });

  it('윤년의 2월에 대해 29일을 반환한다', () => {
    const daysInLeapYearFebraury = getDaysInMonth(2024, 2);
    expect(daysInLeapYearFebraury).toBe(29);
  });

  it('평년의 2월에 대해 28일을 반환한다', () => {
    const dayInNormalYearFebraury = getDaysInMonth(2023, 2);
    expect(dayInNormalYearFebraury).toBe(28)
  });

  it('유효하지 않은 월에 대해 적절히 처리한다', () => {
    const isValidMonth = getDaysInMonth(2024, 13);
    expect(isValidMonth).toBe(31);
  });
});

describe('getWeekDates', () => {
  it('주중의 날짜(수요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const thisWednesday = new Date("2024-11-06");

    const weekDates = getWeekDates(thisWednesday);

    const expectedDates = [new Date("2024-11-03"), new Date("2024-11-04"), new Date("2024-11-05"), new Date("2024-11-06"), new Date("2024-11-07"), new Date("2024-11-08"), new Date("2024-11-09")];
  ...
  it('주의 시작(월요일)에 대해 올바른 주의 날짜들을 반환한다', () => {
    const thisMonday = new Date("2024-11-04");
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연말)', () => {
    const lastDayInYear = new Date("2024-12-31");

    const weekDates = getWeekDates(lastDayInYear);

    const expectedDates = [new Date("2024-12-29"), new Date("2024-12-30"), new Date("2024-12-31"), new Date("2025-01-01"), new Date("2025-01-02"), new Date("2025-01-03"), new Date("2025-01-04")];
  ...
  it('연도를 넘어가는 주의 날짜를 정확히 처리한다 (연초)', () => {
    const firstDayInYear = new Date("2025-01-01");

    const weekDates = getWeekDates(firstDayInYear);
  ...
  it('윤년의 2월 29일을 포함한 주를 올바르게 처리한다', () => {
    const isLeapYear = new Date("2024-02-29");

    const weekDates = getWeekDates(isLeapYear);

    const expectedDates = [new Date("2024-02-25"), new Date("2024-02-26"), new Date("2024-02-27"), new Date("2024-02-28"), new Date("2024-02-29"), new Date("2024-03-01"), new Date("2024-03-02")];
  ...
  it('월의 마지막 날짜를 포함한 주를 올바르게 처리한다', () => {
    const lastDate = new Date("2024-10-31");

    const weekDates = getWeekDates(lastDate);

    const expectedDates = [new Date("2024-10-27"), new Date("2024-10-28"), new Date("2024-10-29"), new Date("2024-10-30"), new Date("2024-10-31"), new Date("2024-11-01"), new Date("2024-11-02")];
...
describe('getWeeksAtMonth', () => {
  it('2024년 7월 1일의 올바른 주 정보를 반환해야 한다', () => {
    ...
    weeks.forEach((week, index) => {
      expect(week).toEqual(expectedWeeks[index]); 
    });
  });
});
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    ...
    expect(isDateInRange(targetDate, rangeEnd, rangeStart)).toBe(false);
  });
});
...
describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    ...
    expect(formatDate(date)).toBe('2024-11-03');
  });

  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    const date = new Date('2024-11-03');
    expect(formatDate(date, 6)).toBe('2024-11-06');
  });
